...
- [ ] **Unify question-JSON schemas so the runtime adapters can be deleted.** Three incompatible shapes coexist: (1) `fill_blanks` legacy nested `questions[].blanks[]` (~80 c_programming questions); (2) `fill_blanks` dict + `{blankN}` placeholders in `description` (canonical, ~36 diskrete_strukturer questions); (3) `multiple_choice` with `options` as a dict `{"A": "..."}` instead of a list (~329 agil_sysudv questions across 6 files). Today these are papered over by `_normalize_options` in `services/question_loader.py`, the dict-vs-nested branch in `grade_fill_blanks`, and the `fill_blanks_segments` pre-processing in `routes/practice.py`. The validator now flags both non-canonical variants as WARNINGs (`non_canonical_options_dict`, `non_canonical_fill_blanks_nested`) — migrate every JSON to one canonical shape, then turn the CI step on with `--strict` and remove the adapters. Both bugs were live in prod until 2026-04-28 because no one noticed for the agil/diskrete exams.
